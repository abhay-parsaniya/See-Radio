...
router.post("/newrequest", clientLoginRequire, (req, res) => {
  ...
  for (let key in formData) {
    ...
    db.query("INSERT INTO newrequest (firstName, lastName, email, contactno, address, city, state, country, zip, companyName, companyEmail, companyContactno, companyScope, companyAddress, companyCity, companyState, companyCountry, companyZip, productName, budget, productScope, advertisementScope, targetViews, file_url, Status) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [formData.firstName, formData.lastName, formData.email, formData.contactno, formData.address, formData.city, formData.clientState, formData.country, formData.zip, formData.companyName, formData.companyEmail, formData.companyContactno, formData.companyScope, formData.companyAddress, formData.companyCity, formData.companyState, formData.companyCountry, formData.companyZip, formData.productName, formData.budget, formData.productScope, formData.advertisementScope, formData.targetViews, secure_url, "Pending"], (err, result) => {
        if(err)
        {
            console.log(err);
        }
...
router.get("/pendingrequest", adminLoginRequire, (req, res) => {
    ...
    db.query("SELECT * FROM newrequest WHERE Status = ?", [status], (err, result) => {
        if(err)
        {
            console.log(err);
        }
...
router.post("/approvedrequest", adminLoginRequire, (req, res) => {
    ...
    db.query("UPDATE newrequest SET Status = ? WHERE idnewrequest = ?", [status, approvedid], (err, result) => {
        if(err)
        {
            ...
            // console.log(result);
            return res.status(200).json({msg: "Your Request is Approved Successfully !!"});
        }
    });
});
